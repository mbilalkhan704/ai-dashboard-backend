...
const { gemini } = require("../utils/constants.js")
...
const sendMessage = async (req, res) => {
    try {
        ...
        if (
            sessionId &&
            !(await ChatSession.findOne({ userId, _id: sessionId }, '_id'))
        ) {
        ...
        const messageResponse = resp.text;
        ...
        if (e instanceof CustomError) {
            return res
                .status(e.statusCode)
                .json(errorResponse(e.message, e.data));
        }
